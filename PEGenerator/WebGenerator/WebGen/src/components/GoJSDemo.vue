<template>
  ...
  <div v-if="menuVisible" @contextmenu.prevent="handleRightClick" class="context-menu" :style="{ top: menuPosition.y + 'px', left: menuPosition.x + 'px' }">
    <ul>
...
const menuAction = (action) => {
  ...
  pegen_graph.addElement(menuPosition.x, menuPosition.y);
...
onMounted(() => {
  ...
  pegen_graph.pg_paper.on('cell:contextmenu', function (cellView, evt) {
    ...
    // // 计算并设置下拉菜单位置
    // const menuEl = dropdownMenu.value.$el.querySelector('.el-dropdown-menu');
    // if (menuEl) {
